...
#define _PROGRES_VERSION "0.0.9-beta"
 ...
 *  @return A pointer to a SignalArray structure containing the loaded input signals.
 ...
 *  @param module Pointer to the Module structure for which to create
 *                dummy inputs.
 *  @return A pointer to a SignalArray structure containing dummy input signals.
 *  @note This function generates default or placeholder input signals for
 *        testing purposes.
 */
 ...
 *  @param outputs Pointer to the SignalArray structure containing output signals.
 *  @note This function will write the output signals to a file
 *        in out custom format.
 */
void save_outputs_to_path(const char* path, SignalArray* outputs);

/** @brief Open a file with the given path and mode, or exit the program
 *         if it fails.
 *  @param path Path to the file to be opened.
 *  @param mode Mode in which to open the file (e.g., "r" for read,
 *              "w" for write).
 *  @return A pointer to the opened FILE object.
 *  @note If the file cannot be opened, this function will print an error
